class LandingPages::PageController < ::Admin::AdminController
  ...
  def import
    ...
    begin
      data = JSON.parse(importer["meta.json"])
    rescue TypeError, JSON::ParserError
      raise LandingPages::ImportError.new
    end
    
    page = LandingPages::Page.find(data["id"])
    
    if page.blank?
      if LandingPages::Page.find_by_path(data["path"])
        raise LandingPages::ImportError.new, I18n.t("themes.import_error.path_exists", path: data["path"])
      end
      
      data["body"] = importer["body.html.erb"]
